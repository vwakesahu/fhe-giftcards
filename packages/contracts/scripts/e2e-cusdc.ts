...
async function main() {
	...
	if (signers.length < 2) {
		throw new Error('Need both PRIVATE_KEY (buyer) and OBSERVER_PRIVATE_KEY set in .env')
	}
	...
	const cUSDC = await CFactory.connect(buyer).deploy(usdcAddress, 'Confidential USDC', 'cUSDC')
	...
	const sigill = await SigillFactory.connect(buyer).deploy(await cUSDC.getAddress())
	...
	await (await (usdc.connect(buyer) as any).approve(await cUSDC.getAddress(), WRAP_AMOUNT)).wait()
	...
	await (await (cUSDC.connect(buyer) as any).approve(sigillAddress, encApprove)).wait()
	...
	const placeLog = placeReceipt!.logs.find((log: any) => {
		try {
			return (
				sigill.interface.parseLog({ topics: log.topics as string[], data: log.data })?.name ===
				'OrderPlaced'
			)
		} catch {
			return false
	...
	if (paid < expectedPrice) {
		console.log(`  Payment short (${paid} < ${expectedPrice}) — rejecting order`)
	...
	const fulfillTx = await (sigill.connect(observer) as any).fulfillOrder(orderId, encAesKey, ipfsCid)
	...
	const aesKeyValue = await tryUnseal<bigint>(finalOrder.encAesKey, FheTypes.Uint128)
	if (aesKeyValue === null) {
	...
	const requestTx = await (cUSDC.connect(observer) as any).requestUnwrap(encUnwrapAmount)
	...
	console.log(`  Unwrap #${unwrapId} requested, awaiting FHE decrypt...`)

	// Poll claimUnwrap — the FHE network needs a few seconds to process the decrypt task.
	let claimed = false
	for (let i = 1; i <= 20; i++) {
		try {
			const claimTx = await (cUSDC.connect(observer) as any).claimUnwrap(unwrapId)
			await claimTx.wait()
	...
	if (!claimed) {
		console.log('  Unwrap still pending after 100s. Observer can call claimUnwrap later.')
	} else {
		...
		console.log(`  Delta: +${Number(observerUsdcAfter - observerUsdcBefore) / 1e6} USDC`)
	}
	...
	console.log('✓ ETH movement — only 0.01 bond visible; payment is cUSDC-encrypted')
}
